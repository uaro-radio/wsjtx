...
ground and nearby objects, relative to cold sky. Such measurements are
widely used for optimizing a station's capabilities for
Earth-Moon-Earth (EME) communication. In addition, you can transmit
and receive lunar echoes of very short alphanumeric messages for
demonstration and amusement purposes.
...
_WSJT-X_ switches to to receive, records the lunar echo arriving about
2.5 s later, computes its spectrum, and displays results. The program
takes care of necessary T/R switching and frequency adjustments for
Doppler compensation. To make such measurements, be sure to check
*Enable VHF and submode features* on the *File | Settings | General*
tab.  Set *Split Operation* to *Rig* or *Fake It* on the *Settings |
Radio* tab, *Doppler tracking* and *Own Echo* or *Constant frequency
on Moon* on the Astronomical Data window, and *Fixed Tone* on the
mwin window. Point your antenna at the Moon and click *Enable Tx* to
start a sequence of echo measurements.
...
measurements appear in the main window. If echoes are strong enough
they will be visible in the waterfall, centered at 1500 Hz. Mote that
a spinner control labeled *Avg* (to the left of the *Decode* button)
controls averaging of successive Echo sequences. Displayed SNR and the
plotted Echo spectrum will be averaged over the selected number of
sequences.

image::Echo_1296.png[align="center",alt="Echo 144 MHz"]

Each echo cycle produces a line of data in the main text window with
...
transmit very short messages (up to six alphanumeric characters) and
receive their echoes, using either multi-tone frequency-shift keying
or standard Morse code.  For example, select *MFSK* and a suitable
tone spacing -- ideally at least one-third of the expected Soppler
spread -- and enter your callsign in the field provided. If your
radio's dial frequency is controlled in 10 Hz steps, you should use
tone spacing of at least 20 Hz.

image::Echo_controls.png[align="center",alt="Echo controls"]

After you click Enable Tx, _WSJT-X_ will send a sequence of six tones,
each lasting 0.341 s, to convey the characters in the entry
...
0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ.  Note that when *MFSK* is
selected, with *Avg* set to anything larger than 1 the decoded echo
message will accumulate indefinitely.

image::Echo_10GHz.png[align="center",alt="Echo 10 GHz"]

Select *CW* to send a short message by Morse code, repeated on the
...
very simple, such as “O” or “OO”.

System timing details are important in Echo mode: its features work
best when the station's T/R switchover times are no longer than a few
hundred milliseconds in each direction. In most amateur EME setups
these times are determined by delays explicitly programmed in the
software, response time of CAT-controlled transceivers, and any
hardware sequencers that are used.

Most features in Echo mode require Doppler compensation via CAT
control of your radio, and you may find it important to experiment
with software options that affect system timing. Use the radio’s
Monitor function (or listen on another receiver) to make sure that in
*MFSK* mode you transmit all six tones at their full programmed
length. Try sending message sequence “07ELSZ”, for example. The
software *Tx delay* (set on the *File | Settings | Advanced* tab)
should be no larger than necessary. Experiment with using *Split
Operation* set to both *Rig* or *Fake It* on the *File | Settings |
Radio* tab. On the *Astronomical Data* window, you might find that
Doppler tracking by *Own Echo* works better for Echo mode than
