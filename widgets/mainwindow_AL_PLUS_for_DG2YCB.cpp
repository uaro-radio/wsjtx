...
// DG2YCB only:  allow double-clicking on combined messages on the standard FT8/FT4 sub-bands
...
void MainWindow::check_button_color()
{
    ...
    if (!m_config.button_coloring_disabled()) {
      ...
      if (ui->houndButton->isChecked() && !m_config.button_coloring_disabled()) {
          ...
          if (m_useDarkStyle) {
             ui->houndButton->setStyleSheet("QPushButton {background-color: #505F69; color: #ffffff; border: 1px solid #32414B; color: #F0F0F0; border-radius: 4px; padding: 3px; outline: none;}");
          } else {
             ui->houndButton->setStyleSheet("QPushButton {background-color: #e1e1e1; color: #000000; border: 1px solid #adadad; border-radius: 0px; padding: 3px; outline: none;}");
          }
      }
      if (m_mode=="FT8") {
