...
MainWindow::MainWindow(QDir const& temp_directory, bool multiple,
  ...
  m_messageClient {new MessageClient {QApplication::applicationName (),
        ...
        this}},
  m_psk_Reporter {&m_config, QString {"WSJT-X v" + version () + " ifox+"}.simplified ()},     // UR
...
//      ui->tabWidget->setMaximumHeight(210);                           // UR for AL
      if (ui->actionUse_Dark_Style->isChecked()) {
//          ui->tabWidget->setMaximumHeight(235);                       // UR for normal + widescreen
          ui->foxButton->setMinimumWidth(50);                         // UR for normal + widescreen
...
//          ui->tabWidget->setMaximumHeight(235);                       // UR for normal + widescreen
          ui->foxButton->setMaximumWidth(40);                         // UR for normal + widescreen
          ...
          ui->foxButton->setMinimumWidth(0);                          // UR for normal + widescreen
  ...
  if(bmsk144 and (line[0]!=0)) {
    ...
    QTimer::singleShot (100, [=] {                       // UR delete for versions without alerts
      if (m_config.alert_Enabled() && ((m_config.alert_DXcall() && play_DXcall && m_hisCall!="") or (m_config.alert_Wanted() && play_Wanted))) {
        ...
        if (m_config.alert_DXcall() && play_DXcall) effect1->setFileName(QString("%1/%2").arg(binPath, "/sounds/DXcall.wav"));
        else if (m_config.alert_Wanted() && play_Wanted) effect1->setFileName(QString("%1/%2").arg(binPath, "/sounds/Wanted.wav"));
        effect1->open(QIODevice::ReadOnly);
        audio->start(effect1);
#else
        QString binPath = QCoreApplication::applicationDirPath();
        if (m_config.alert_DXcall() && play_DXcall) QSound::play(binPath + "/sounds/DXcall.wav");  // for Linux and macOS
        else if (m_config.alert_DXcall() && play_Wanted) QSound::play(binPath + "/sounds/Wanted.wav");  // for Linux and macOS
#endif
        play_DXcall = false;
        play_Wanted = false;
      }
    ...
    if (!m_config.button_coloring_disabled()) {
      ...
      if (ui->foxButton->isChecked()) {
          ...
          if (m_useDarkStyle) {
             ...
             ui->foxButton->setStyleSheet("QPushButton {background-color: #e1e1e1; border: 1px solid #adadad; border-radius: 0px; padding: 3px; outline: none; min-width: 3em;}");
          }
      }
