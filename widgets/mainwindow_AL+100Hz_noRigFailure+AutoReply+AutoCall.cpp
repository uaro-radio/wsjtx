...
/*  qreal pointSize = m_config.text_font().pointSizeF();                // UR disable for AL
  ...
  ui->respondComboBox->addItem("Auto Reply");
  ui->respondComboBox->addItem("Auto Call");
...
//  ::memcpy(dec_data.params.hiscall,(Radio::base_callsign (hisCall) +  "            ").toLatin1 ().constData (), sizeof dec_data.params.hiscall);
  ...
  if(bmsk144 and (line[0]!=0)) {
    ...
    // Auto Reply for MSK144
    if (decodedtext.string().contains(m_config.my_callsign()) && decodedtext.string().contains(" " + m_hisCall + " ") &&
       (m_hisCall!="") && !decodedtext.string().contains("73 ") && (ui->respondComboBox->currentText()=="Auto Reply")) {
          m_bDoubleClicked = true;
          processMessage(decodedtext);
          auto_tx_mode(true);
          QTimer::singleShot ((int(12000.0*m_TRperiod)), [=] {auto_tx_mode(false);});   // Auto Reply Tx max 12*TRperiod
    }
    // Auto Call for MSK144
    if (decodedtext.string().contains(" " + m_hisCall + " ") && (ui->respondComboBox->currentText()=="Auto Call") &&
        (m_hisCall!="") && !no_auto_call && (decodedtext.string().contains("CQ ") or decodedtext.string().contains("73 "))) {
          m_bDoubleClicked = true;
          processMessage(decodedtext);
          auto_tx_mode(true);
          QTimer::singleShot ((int(6000.0*m_TRperiod)), [=] {    // Auto Call Tx max 6*TRperiod, then Auto Reply
              auto_tx_mode(false);
//                  no_auto_call=true;
              ui->respondComboBox->setCurrentIndex(3);
          });
    }
    // CQ First for MSK144
      ...
      if(!bAvgMsg) {
        ...
        if ((m_mode=="FT8" or m_mode=="FT4" or m_mode=="Q65" or m_mode=="FST4") &&
             ...
             (m_hisCall!="") && !decodedtext.string().contains("73 ") && (ui->respondComboBox->currentText()=="Auto Reply"))) {
                ...
                auto_tx_mode(true);
                QTimer::singleShot ((int(8000.0*m_TRperiod)), [=] {auto_tx_mode(false);});   // Auto Reply Tx max 8*TRperiod
        }

          // Auto Call
        if ((m_mode=="FT8" or m_mode=="FT4" or m_mode=="Q65" or m_mode=="FST4") && !no_auto_call &&
            (decodedtext.string().contains(" " + m_hisCall + " ") && (ui->respondComboBox->currentText()=="Auto Call") &&
            (m_hisCall!="") && (decodedtext.string().contains("CQ ") or decodedtext.string().contains("73 ")))) {
              m_bDoubleClicked = true;
              processMessage(decodedtext0);
              auto_tx_mode(true);
              QTimer::singleShot ((int(6000.0*m_TRperiod)), [=] {    // Auto Call Tx max 6*TRperiod, then Auto Reply
...
/*
              QTimer::singleShot ((int(12000.0*m_TRperiod)), [=] {    // Auto Call Tx pause 18*6*TRperiod
// klappt nicht                  if (m_inQSOwith="") no_auto_call=false;
                  no_auto_call=false;
              });
              QTimer::singleShot ((int(18000.0*m_TRperiod)), [=] {    // stop Auto Call, then Auto Reply
                  no_auto_call=true;
                  ui->respondComboBox->setCurrentIndex(3);
              });
*/
...
//  ui->dxGridEntry->clear();    // UR disabled because not useful with highlightDXCall/DXGrid feature
  no_auto_call=false;          // Reset any auto call blocking
  statusChanged();
  statusUpdate ();
}
