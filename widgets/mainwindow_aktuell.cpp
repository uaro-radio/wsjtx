      ...
      if(!bAvgMsg) {
        if(m_mode=="FT8" and SpecOp::FOX == m_specOp) {
          ...
          if ((m_mode=="FT8" or m_mode=="FT4" or m_mode=="Q65" or m_mode=="FST4") && (m_hisCall!="") &&
              ...
              && (m_config.Wait_features_enabled() or m_specOp==SpecOp::HOUND)) {  // URUR
                  tx_watchdog (false);
                  m_bDoubleClicked = true;
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  int nmod = fmod(double(message.timeInSeconds()),2.0*m_TRperiod);
  m_txFirst=(nmod!=0);
  if(SpecOp::HOUND == m_specOp && !m_bDoubleClicked) m_txFirst=false;   //Hound usually transmits first   // URUR
